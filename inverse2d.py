...
def generate_interior_data(path="receiver_fourier.csv"):
    ...
    u_data_np = df[['Magnitude']].values  # u: valor de la solución
...
def generate_collocation_points(N_interior=1500, N_neumann = 200, N_boundary=200, std_dev=0.2, csv_path='ricker_fourier.csv'):
    ...
    """
    x_int = np.random.normal(loc=0.5, scale=0.3, size=(N_interior, 1))
    y_int = -np.random.exponential(scale=0.3, size=(N_interior, 1))
    ...
    magnitudes = df["Magnitude"].values[:N_boundary].reshape(-1, 1)
    ...
    return X_int, X_bnd, u_bnd, X_neumann
###############################################################################
...
def loss_pde_inverse(model, X_int):
    ...
    grads_k = derivative(c, X_int, order=1)
    k_x = grads_k[:, 0:1]
    k_y = grads_k[:, 1:2]
    ...
    residual = (omega**2/c**2)*u + (u_xx + u_yy) - forcing
...
def plot_solution_and_k(model, epoch, folder="figs_inverse_mixed", n_points=150):
    ...
    # Plot for PINN k (epoch {epoch})
    ax2 = fig.add_subplot(1, 2, 2)
    im2 = ax2.pcolormesh(X_mesh, Y_mesh, k_pred, cmap='viridis', shading='auto')
    ax2.set_title(f"PINN k (epoch {epoch})")
...
def train_inverse_pinn_mixed(
    ...
    for epoch in tqdm(range(1, adam_epochs+1), desc="Adam"):
        ...
        total_loss = pde_loss + lambda_bc * bc_loss_val + lambda_data * data_loss_val + neumann_loss
        total_loss.backward()
        optimizer_adam.step()
    ...
    def closure():
        ...
        bc_loss_val = loss_bc(model, X_bnd,u_bnd)
        data_loss_val = loss_data(model, X_data, u_data)
        total_loss = pde_loss + lambda_bc * bc_loss_val + lambda_data * data_loss_val
        total_loss.backward()
        return total_loss
    ...
    for i in tqdm(range(1, lbfgs_iterations+1)):
        ...
        if (i+1) % 100 == 0 or (i+1) == lbfgs_iterations:
            ...
            print(f"  [LBFGS iter {i+1:5d}] total_loss={current_total:.4e}, "
                  f"pde_loss={current_pde:.4e}, bc_loss={current_bc:.4e}, "
                  f"data_loss={current_data:.4e}")
            plot_solution_and_k(model, adam_epochs + i + 1, folder="figs_inverse_mixed_lbfgs")
...
if __name__ == "__main__":
    ...
    X_int, X_bnd, u_bnd, X_neu = generate_collocation_points(N_interior=50000, N_boundary=100, N_neumann=500, csv_path="ricker_fourier.csv")
    X_data, u_data = generate_interior_data(path="receiver_fourier.csv")
